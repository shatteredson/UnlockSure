...
// Constants / defaults
// ----------------------
if (! defined('UNLOCKSURE_IMEI_CACHE_TTL')) define('UNLOCKSURE_IMEI_CACHE_TTL', 12 * HOUR_IN_SECONDS); // 12 hours *24 when live
if (! defined('UNLOCKSURE_IMEI_RATE_LIMIT_PER_HOUR')) define('UNLOCKSURE_IMEI_RATE_LIMIT_PER_HOUR', 30); // default 30 requests/hour per IP *60 when live
...
function unlocksure_imei_shortcode($atts = [] ) { 
    ...
    $html .= '    <h1 class="us-title">Free IMEI Checker</h1>';
    ...
    $html .= '      <input id="unlocksure-imei-input" name="imei" type="text" maxlength="15" ';
    ...
    $html .= '    <div class="us-card"><h3>What We Check</h3><ul>';
    $html .= '      <li>Current Carrier</li><li>SIM Lock Status</li><li>Blacklist Status</li><li>Brand & Model</li>';
    $html .= '    </ul></div>';
    $html .= '    <div class="us-card"><h3>Why It Matters</h3><ul>';
    $html .= '      <li>Confirms if we can unlock</li><li>Sets accurate ETA</li><li>Prevents order mistakes</li>';
    $html .= '    </ul></div>';
    $html .= '  </div>';
    $html .= '</section>';
...
function unlocksure_enqueue_imei_assets() {
	...
	// Load brand fonts for this page only
	wp_enqueue_style(
		'unlocksure_fonts',
